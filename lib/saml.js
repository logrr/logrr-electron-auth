var crypto = require('crypto'),
    ...
    InMemoryCacheProvider = require('./inmemory-cache-provider.js').CacheProvider,
    Q = require('q');
...
SAML.prototype.setParams = function (options) {
  this.options = this.initialize(options);
  this.cacheProvider = this.options.cacheProvider;
};

SAML.prototype.getOptions = function () {
  return this.options;
};
...
SAML.prototype.isSAMLResponse = function (req) {
  return (req.body && req.body.SAMLResponse);
};
...
SAML.prototype.generateAuthorizeRequest = function (idurl, nameID, isPassive) {
  var self = this;
  var id = "_" + self.generateUniqueID();
  var instant = self.generateInstant();
  var protocol = self.options.protocol || 'http://';
  var callbackUrl;
  var forceAuthn = self.options.forceAuthn || false;
    ...
    if (self.options.callbackUrl) {
      ...
      callbackUrl = protocol + idurl + self.options.path;
    }
    ...
    if (nameID) {
      request['samlp:AuthnRequest']['saml:Subject'] = {
        '@xmlns:saml' : 'urn:oasis:names:tc:SAML:2.0:assertion',
        'saml:NameID' : {
          '@xmlns:saml' : 'urn:oasis:names:tc:SAML:2.0:assertion',
...
SAML.prototype.validatePostResponse = function (container, callback) {
  var self = this;
  var xml = new Buffer(container, 'base64').toString('utf8');
...
SAML.prototype.processValidlySignedAssertion = function(xml, inResponseTo, callback) {
  ...
  .then(function(){
    ...
    if (attributeStatement) {
      ...
      attributeStatement.forEach(function (statement) {
        attributes.push(statement.Attribute[0]);
      });
